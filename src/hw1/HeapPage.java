...
public class HeapPage {
	...
	public int getNumSlots() {
		int i = this.td.getSize();
		int cap = HeapFile.PAGE_SIZE;
		return (int) (8 * cap / (8 * i + 1));
	}
	...
	public void addTuple(Tuple t) throws Exception {
		if(! t.td.equals(this.td)) {
			throw new Exception();
		}
		
		boolean flag = true; 
		for(int i = 0; i < header.length; i++) {
			if (header[i] != (byte)(2^7 - 1)) {
				flag = false;
	...
	public void deleteTuple(Tuple t) throws Exception {	
		if(t.getPid() == this.getId() && t.getId() != -1) {
		...
		for(int i = 0; i < this.getNumSlots(); i++) {
			if(this.tuples[i].equals(t)) {
				if(this.slotOccupied(i) && this.tuples[i].getPid() == this.getId()) {
					flag = false;
	...
	public Iterator<Tuple> iterator() {
		ArrayList<Tuple> tupleC = new ArrayList<Tuple>();
		int numSlots = this.getNumSlots();
		for (int i = 0; i < numSlots; i++) {
			if(this.slotOccupied(i)) {
				tupleC.add(this.tuples[i]); 
			}
		}
		return tupleC.iterator();
	}
	...
	public void resetHeader() {
		for(int i = 0; i < this.getHeaderSize(); i++) {
			this.header[i] = (byte) 0;
		}
	}
	
	/**
	 * Set the ID for the HeapPage
	 */
